...
int main(int argc, char* argv[])
{
    ...
    for(;;)
    {
		...
		//push onto thread safety queue
		//big function will be a function pointer for each thread
		//inside of function pop it
		...
		sprintf(fullpath,"%s%s",argv[3], path);
		printf("fullpath %s\n",fullpath);
					...
					while ((dp = readdir(dirp)) != NULL){
						...
						if(strcmp(dp->d_name,".") == 0 || strcmp(dp->d_name,"..") == 0){
							...
							if(strcmp(path,"") == 0){
								...
								//char * pch = strstr(fullpath,argv[2]);
								...
								//memmove(&path[0], &path[0 + 1], strlen(path) - 0);
								//sprintf(temp,"http://%d:%d/", Address.sin_addr.s_addr, ntohs(Address.sin_port));
								//sprintf(temp,
								thing.append(std::string("<p><a href=\"") + dp->d_name + "\">" + dp->d_name + "</a></p>\n");
							}
							else {
