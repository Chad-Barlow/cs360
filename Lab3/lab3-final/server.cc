...
//Data package used to pass to each thread
struct package{
...
void *runThread(void *p){
    ...
    tmp++;
    counter = tmp;
    if(debugging){printf("Counter value: %d\n",counter);}
	...
	for(;;){	
		...
		if(connectionQ.getSize()>0){
			...
			if(debugging){printf("Got from browser %d\n%s\n",rval, pBuffer);}
			#define MAXPATH 1000
			char path[MAXPATH];
			rval = sscanf(pBuffer,"GET %s HTTP/1.1",path);
			if(debugging){printf("Got rval %d, path %s\n",rval,path);}
			char fullpath[MAXPATH];
			sprintf(fullpath,"%s%s",argv, path);
			if(debugging){printf("fullpath %s\n",fullpath);}
			...
			if(stat(fullpath, &filestat)) {
				...
				if(S_ISREG(filestat.st_mode)) {
					memset(pBuffer,0,sizeof(pBuffer));
					if(debugging){cout << path << " is a regular file \n";}
					if(debugging){cout << "file size = "<<filestat.st_size <<"\n";}
					...
					if(debugging){cout << path << " is a DIRECTORY\n";}
				 	string thing = "";
					//path + "/index.html";
					...
					if(stat(newArray, &filestat) == 0){	
						...
						char *buffer = (char *)malloc(sizeof(char)*lSize);
						fread(buffer, 1, lSize,fp);
						if(debugging){cout << "Servering index.html\n";}
						memset(pBuffer,0,sizeof(pBuffer));
						sprintf(pBuffer,"HTTP/1.1 200 OK\r\n\
							Content-Type: text/html\r\n\
							Content-Length: %lu\
